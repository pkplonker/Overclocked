...
namespace Stuart
{
	public abstract class Bench : MonoBehaviour, IInteractable
	{
		...
		protected static Inventory GetInvent(Interactor interactor) =>
			interactor.TryGetComponent<Inventory>(out var invent) ? invent : null;
		...
		private void SpawnItem()
		{
			...
			currentSpawnedItem = Instantiate(CurrentItem.GetPrefab(), itemSpot.position,
				currentItem.GetPrefab().transform.rotation, itemSpot);
